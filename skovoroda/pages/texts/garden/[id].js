...
export default function SkovorodaSadPage({ sadData, allSongsShortData, selectedId, deviceEnding }) {
  ...
  function getPrevSongIdOrDefault() {
    const translationId = getTranslationId(selectedId);
    let prevSongId = +getSongId(selectedId) - 1;
    if (prevSongId < 1) {
      return undefined;
    }
    if (!translationId) {
    ...
    while (prevSongId >= 1) {
      const prevShortData = allSongsShortData.find(data => data.songId == prevSongId);
      if (!prevShortData) {
        return undefined;
      }
      const isPrevTranlateExist = prevShortData.translates.some(translation => translation.translationId === translationId);
      if (isPrevTranlateExist) {
        return prevSongId;
      }
      prevSongId--;
    }
  ...
  function getNextSongIdOrDefault() {
    const translationId = getTranslationId(selectedId);
    let nextSongId = +getSongId(selectedId) + 1;
    if (nextSongId > 30) {
      return undefined;
    }
    if (!translationId) {
    ...
    while (nextSongId <= 30) {
      const nextShortData = allSongsShortData.find(data => data.songId == nextSongId);
      if (!nextShortData) {
        return undefined;
      }
      const isNextTranlateExist = nextShortData.translates.some(translation => translation.translationId === translationId);
      if (isNextTranlateExist) {
        return nextSongId;
      }
      nextSongId++;
    }
  ...
  function prevSongClicked() {
    const prevSongId = getPrevSongIdOrDefault();
    if (prevSongId) {
      selectDropdownSadSong(prevSongId);
    }
  }

  function nextSongClicked() {
    const nextSongId = getNextSongIdOrDefault();
    if (nextSongId) {
      selectDropdownSadSong(nextSongId);
    }
  }

  function isPrevSongExists() {
    return getPrevSongIdOrDefault() ? true : false;
  }

  function isNextSongExists() {
    return getNextSongIdOrDefault() ? true : false;
  }
